...
do
	local path = string.match(debugstack(1, 1, 0), "AddOns[\\/](.+)HFSLibGraph%-2%.0%.lua")
...
function GraphFunctions:AddDataSeries(points, color, n2, linetexture, label, precision)
	label = label or ""
	local data
	...
	tinsert(self.Data,{Points = data; Color = color; LineTexture=linetexture; LineLabel=label; LabelPrecision=precision})
	...
	if self.XGridInterval then
		...
		for i = LowerXGridLine, UpperXGridLine do
			if i ~= 0 or not self.XAxisDrawn then
				--THIS IS HARDCODED TO TUESDAY LINES AND REQUIRES INTERVAL TO BE 1. BE CAREFUL WITH THIS.
				local startdate = 60 * 60 * 24 * 365.25 * 52
				local xcoord = i * self.XGridInterval * 60 * 60 * 24
				local currdate = startdate + xcoord
				if (date("%a", currdate) == "Tue") then
				...
				if NoSecondary or math_fmod(i, self.GridSecondaryX) == 0 then
					...
					F:SetTextColor(1, 1, 1)
					F:ClearAllPoints()
					F:SetPoint("BOTTOM", T, "BOTTOM", -2, 2)
					
					
					F:SetText(date("%m/%d/%Y", currdate))
					F:Show()
				end
			end
		end
...
function GraphFunctions:RefreshLineGraph()
	...
	for k1, series in pairs(self.Data) do
		local FirstPoint, LastPoint, LastDiff, LastY, T = nil
		FirstPoint = true
		LastPoint = nil
		LastDiff = nil
		...
		for k2, point in pairs(series.Points) do
			if LastPoint then
				...
				T = self:DrawLine(self, LastPoint.x, LastPoint.y, TPoint.x, TPoint.y, 32, series.Color, nil, series.LineTexture, k1)

				if (FirstPoint and T ~= nil) then
					F = self:FindFontString()
					F:SetFontObject("GameFontHighlightSmall")
					F:SetTextColor(series.Color[1], series.Color[2], series.Color[3])
					F:ClearAllPoints()
					F:SetPoint("BOTTOMRIGHT", T, "LEFT", (LastPoint.y - TPoint.y)/-10, (LastPoint.y - TPoint.y)/2 - 5)
					F:SetText(series.LineLabel)
					F:Show()
					FirstPoint = false
				end
				LastDiff = TPoint.y - LastPoint.y
				LastPoint = TPoint
			else
		...
		if (T ~= nil) then
			...
			F:SetTextColor(series.Color[1], series.Color[2], series.Color[3])
			F:ClearAllPoints()
			F:SetPoint("BOTTOMRIGHT", T, "RIGHT", LastDiff/-10 + 35, (LastDiff / 2) - 5) -- X 35
			F:SetText(round(LastY, series.LabelPrecision))
			F:Show()
		end
	end
...
function round(val, decimal)
	...
	local postdec = ""
	if (decimal) then
		num = math.floor( (val * 10^decimal) + 0.5) / (10^decimal)
	else
		decimal = 0
		num = math.floor(val+0.5)
	end
	
	for i = 1, 1 * decimal do
		if ((num * (10^decimal)) % 10^i == 0) then
			postdec = postdec .. "0"
		end
	end
	...
	if (decimal == 0) then
		...
		if ((num * (10^decimal)) % 10^decimal == 0) then
			return num .. "." .. postdec
		else
			return num .. postdec
		end
...
function lib:DrawLine(C, sx, sy, ex, ey, w, color, layer, linetexture)
	...
	elseif C.CustomLine then --overall chart texture
		...
		T:SetTexture("Interface\\Addons\\ItemLevelHistory\\Media\\line.tga")
	end
...
function lib:DrawVLine(C, x, sy, ey, w, color, layer)
	...
	local T = tremove(C.GraphLib_Lines) or C:CreateTexture(nil, "ARTWORK")
	T:SetTexture("Interface\\Addons\\ItemLevelHistory\\Media\\sline.tga")
	tinsert(C.GraphLib_Lines_Used, T)
